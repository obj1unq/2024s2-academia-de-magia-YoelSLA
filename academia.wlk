class Academia {
	...
	method puedeGuardar(cosa) = (!self.estaGuardada(
		cosa
	)) && self.hayAlgunMuebleParaGuardar(cosa)
	
	method hayAlgunMuebleParaGuardar(cosa) = muebles.any(
		{ m => m.puedeGuardar(cosa) }
	)
	
	method mueblesDisponibles(cosa) = muebles.filter({ m => m.puedeGuardar(cosa) })
...
class Baul inherits Mueble {
	...
	override method puedeGuardar(
		cosa
	) = (self.volumenTotal() + cosa.volumen()) <= volumenMaximo
...
class Armario inherits Mueble {
	...
	override method puedeGuardar(cosa) = cosas.size() < cantidadMaxima
